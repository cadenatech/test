...
function injectOverlayResponse(blob, payload) {
  return blob.text().then(function (html) {
    ...
    var overlay = [
      ...
      'function buildMetaToken(meta){if(!meta)return"";var parts=[];if(meta.etag)parts.push("e:"+meta.etag);if(meta.lastModified)parts.push("m:"+meta.lastModified);if(meta.size)parts.push("s:"+meta.size);return parts.join("|");}',
